...
TEST(BasicOperation, Div_Simple) {
    ...
    y.Backward();
    ASSERT_EQ(x1.GetGrad()->GetData(), 0.5);
...
TEST(BasicOperation, Div_SameVariable) {
    ...
    y.Backward();
    ASSERT_EQ(x1.GetGrad()->GetData(), 0);
}
...
TEST(BasicOperation, Div_MultiVariable) {
    ...
    ASSERT_EQ(y.GetData(), 1);
    y.Backward();
    ASSERT_EQ(x1.GetGrad()->GetData(), 0.125);
    ASSERT_EQ(x2.GetGrad()->GetData(), -0.25);
    ASSERT_EQ(x3.GetGrad()->GetData(), -0.5);
}
...
TEST(BasicOperation, Div_RightScalar) {
    ...
    ASSERT_EQ(y.GetData(), 2);
    y.Backward();
    ASSERT_EQ(x.GetGrad()->GetData(), 0.5);
}
